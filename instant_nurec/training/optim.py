...
from instant_nurec.config_schema.train import KelvinOptimizerConfig, KelvinSchedulerConfig
...
class CosineWithWarmupPBScheduler(torch.optim.lr_scheduler.LRScheduler):
    """Exact public port of Bazel NRE's progress-based Kelvin schedule."""

    def __init__(self, optimizer: torch.optim.Optimizer, config: KelvinSchedulerConfig, last_epoch: int = -1):
    ...
    def get_lr(self) -> list[float]:
        ...
        if self._total_local_steps <= 0 or self._total_epochs <= 0:
            raise RuntimeError("set_progress() must be called before stepping the Kelvin scheduler")
...
def make_kelvin_optimizer(
    parameters,
    config: KelvinOptimizerConfig,
    ...
    if config.implementation in ("auto", "apex-fused-adam"):
        try:
            ...
            if config.implementation == "apex-fused-adam":
                raise RuntimeError(
                    "Exact Bazel optimizer parity requires apex.optimizers.FusedAdam; install NVIDIA Apex "
