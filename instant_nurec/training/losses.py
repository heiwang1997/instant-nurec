...
from instant_nurec.config_schema.train import KelvinLossConfig
...
from instant_nurec.training.renderer import KelvinRenderOutput
...
def _weighted_masked_mean(
    ...
    """Match Bazel semantic weighting: zero weighted-out values but retain them in the denominator."""
...
def _semantic_targets(labels) -> torch.Tensor:
    ...
    # Exact Bazel priority (last write wins): ROAD < SKY < MOVABLE < EGO.
...
def _required(value, name: str):
    if value is None:
        raise RuntimeError(f"Configured Kelvin loss requires {name}, but it is unavailable")
    return value


class KelvinLosses(torch.nn.Module):
    """Loss profile matching Kelvin DAv3 context/render Bazel configs."""

    def __init__(self, config: KelvinLossConfig):
        super().__init__()
        self.config = config
        self._lpips = None
        if config.lpips > 0:
    ...
    def _render_losses(
        self,
        output: KelvinRenderOutput | None,
    ...
    def forward(
        ...
        pack: KelvinSupervisionPack,
        context: DataAndRenderingBatch,
        output: KelvinRenderOutput | None = None,
        ...
        if not values:
            raise RuntimeError("No Kelvin loss is active for this batch; check labels and loss weights")
        total = sum(value * float(weights[name]) for name, value in values.items())
        return KelvinLossReturn(total_value=total, values=values, skipped=tuple(context_skipped + render_skipped))
