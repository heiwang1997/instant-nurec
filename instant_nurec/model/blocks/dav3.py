...
def convert_dav3_state_dict(
    ...
    *,
    ...
    """Map an official Depth Anything V3 checkpoint into Kelvin modules."""
    ...
    for key, value in state_dict.items():
        ...
        if mapped is None and key.startswith("model.backbone.pretrained."):
            internal_key = key.removeprefix("model.backbone.pretrained.")
            if internal_key == "cls_token":
                mapped = {"BACKBONE.cls_tokens": value.squeeze(1)}
            elif internal_key == "pos_embed":
